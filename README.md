...
Ce repo ne contient pas de code applicatif. Il référence les sous-projets via des git submodules et centralise la documentation de planification.

L'infra prod est gérée par **Dokploy** (self-hosted PaaS) sur un VPS OVH — Traefik intégré (HTTPS Let's Encrypt auto), UI web, webhook GitHub → redeploy auto. Il n'y a plus de submodule `infra` dédié : voir `CLAUDE.md` section "Infra prod" pour le détail.
