...
Brevo does not host `contact@cloudbreak-app.com`. **ImprovMX** is the chosen free MX/forwarding
layer for that address (Cloudflare Email Routing was considered but not used): ImprovMX hosts
the MX records for `cloudbreak-app.com` and forwards `contact@cloudbreak-app.com` to
`contact.cloudbreak@gmail.com`, an operator-controlled Gmail relay mailbox — not a personal inbox.

```text
contact@cloudbreak-app.com  --ImprovMX MX/forward-->  contact.cloudbreak@gmail.com
```

This is inbound-only routing (receiving support mail); it is entirely separate from the
outbound Brevo sending path used for Auth emails above, and separate from the development
sending rollout. Configuration lives in ImprovMX's dashboard and OVH DNS (MX + the TXT record
ImprovMX requires) — no credential or secret is stored in this repo for this route.
...
reauthentication and permanent e-mail change) before requesting delivery. Password reset
currently remains a French-only fallback because the Supabase mobile SDK does not accept
custom metadata in `resetPasswordForEmail`.
...
`{{ .ConfirmationURL }}` is incompatible with the mobile screen. The current template is
French-only (no `.Data.locale` conditional), so every recovery email has a reliable French
fallback while reset remains on the SDK-supported `resetPasswordForEmail(email)` call.

The saved Reset Password template contract is currently:

- Subject: `Réinitialise ton mot de passe, Cloudbreak Mer de nuage`
- Body: exactly one French block, with exactly one `{{ .Token }}` placeholder:
...
This template must contain no `{{ .ConfirmationURL }}`, `.Data.locale`, `Reset Password`, or
`Follow this link` text. Supabase Auth renders the saved source; Brevo transports the rendered
message and does not concatenate template branches. After any Dashboard edit, reload **Reset
Password** in source mode and verify the exact contract above before requesting another reset.
The previously received franglais and duplicated French blocks were caused by the legacy English
link source plus duplicated conditional content still present in the hosted template; they were
not generated by the mobile screen.
...
For recovery, Supabase renders the static French reset template. The current app language
does not change this message. A future transactional email service or supported Supabase
metadata option would be required before reset can follow the pre-auth app locale.
...
8. Configure **Reset Password** with `{{ .Token }}`, then request a reset for a controlled
   account. Confirm receipt of one French block containing one six-digit code, complete
   `verifyOtp(..., type: 'recovery')` in the app, set a new password, and verify that the old
   password is rejected while the new password signs in successfully. Changing the app language
   or account metadata must not be reported as changing this reset message.
